...
class BltModel {
    ...
    public function getCalonPenerima($id_program) {
        $this->db->query('SELECT c.id_calon, c.id_warga, c.id_program, c.tanggal_usulan, c.status, c.created_at, c.updated_at, w.nik, w.nama_lengkap, w.alamat, w.penghasilan, w.jumlah_tanggungan, w.kondisi_rumah, w.pekerjaan FROM calon_penerima c JOIN warga w ON c.id_warga = w.id_warga WHERE c.id_program = :id_program ORDER BY c.created_at DESC');
        ...
        foreach ($calon as &$c) {
            ...
            foreach ($nilaiList as $n) {
                $c['nilai'][(int)$n['id_kriteria']] = (float)$n['nilai_asli'];
            }
        }
    ...
    public function getCalonById($id_calon) {
        $this->db->query('SELECT c.id_calon, c.id_warga, c.id_program, c.tanggal_usulan, c.status, c.created_at, c.updated_at, w.nik, w.nama_lengkap, w.alamat, w.penghasilan, w.jumlah_tanggungan, w.kondisi_rumah, w.pekerjaan FROM calon_penerima c JOIN warga w ON c.id_warga = w.id_warga WHERE c.id_calon = :id_calon');
        $this->db->bind('id_calon', $id_calon);
        return $this->db->single();
    }
