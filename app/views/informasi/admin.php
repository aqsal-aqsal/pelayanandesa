                        ...
                        <?php foreach($data['informasi'] as $i): ?>
                            <tr class="hover:bg-gray-50/50 transition">
                                ...
                                <td class="px-8 py-5 text-right">
                                    <a href="<?= BASEURL; ?>/informasi/hapus/<?= $i['id_informasi']; ?>" onclick="return confirm('Hapus informasi ini?')" class="text-rose-500 hover:text-rose-700 font-black text-[10px] uppercase tracking-widest">Hapus</a>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    <?php endif; ?>
...
<div id="infoModal" class="hidden fixed inset-0 bg-slate-900/40 backdrop-blur-sm overflow-y-auto h-full w-full z-50 transition-all duration-300">
    <div class="relative top-20 mx-auto p-10 border w-[600px] shadow-2xl rounded-[32px] bg-white mb-20">
        <div class="text-center mb-8">
            <h3 class="text-2xl font-black text-slate-900">Buat Informasi Publik</h3>
        </div>
        
        <form action="<?= BASEURL; ?>/informasi/tambah" method="POST" class="space-y-6 text-left">
            <div class="space-y-2">
                <label class="block text-xs font-bold text-gray-400 uppercase tracking-widest">Judul Informasi</label>
                <input type="text" name="judul" required class="block w-full px-5 py-4 bg-gray-50 border border-gray-200 rounded-2xl focus:outline-none focus:ring-2 focus:ring-blue-500 transition">
            </div>

            <div class="space-y-2">
                <label class="block text-xs font-bold text-gray-400 uppercase tracking-widest">Konten / Isi Berita</label>
                <textarea name="konten" rows="8" required class="block w-full px-5 py-4 bg-gray-50 border border-gray-200 rounded-2xl focus:outline-none focus:ring-2 focus:ring-blue-500 transition"></textarea>
            </div>
            ...
            <div class="flex space-x-3 pt-4">
                ...
                <button type="submit" class="flex-[2] py-4 bg-blue-600 text-white font-black rounded-2xl shadow-lg shadow-blue-100 hover:bg-blue-700 transition">Publikasikan Sekarang</button>
            </div>
        </form>
    </div>
</div>
