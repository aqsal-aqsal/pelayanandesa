...
<div class="space-y-8">
    <div class="bg-white p-10 rounded-[32px] shadow-sm border border-gray-100 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
            <h2 class="text-3xl font-black text-slate-900 mb-2">Data Pejabat Desa</h2>
            <p class="text-gray-500">Kelola data pejabat dan upload tanda tangan untuk kebutuhan penandatanganan surat.</p>
        </div>
                            ...
                            <tr class="hover:bg-gray-50/50 transition">
                                ...
                                <td class="px-8 py-5 text-right space-x-2">
                                    <?php if((int)$p['id_petugas'] === (int)$_SESSION['user']['id_user'] || $_SESSION['user']['level'] == 'petugas'): ?>
                                        <button onclick='openTtdModal(<?= json_encode($p); ?>)' class="px-4 py-2 bg-blue-50 text-blue-600 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-blue-100 transition">
                                            Upload TTD
                                        </button>
                                    <?php endif; ?>
                                    <?php if($_SESSION['user']['level'] == 'petugas'): ?>
                                        <button onclick='openPejabatModal("edit", <?= json_encode($p); ?>)' class="px-4 py-2 bg-slate-900 text-white rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-slate-800 transition">
                                            Edit
                                        </button>
                                        <a href="<?= BASEURL; ?>/pejabat/hapus/<?= $p['id_petugas']; ?>" onclick="return confirm('Hapus data pejabat ini?')" class="px-4 py-2 bg-rose-50 text-rose-600 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-rose-100 transition">
                                            Hapus
                                        </a>
                                    <?php endif; ?>
                                </td>
...
<div id="pejabatModal" class="hidden fixed inset-0 bg-slate-900/40 backdrop-blur-sm overflow-y-auto h-full w-full z-50 transition-all duration-300">
    <div class="relative top-20 mx-auto p-10 border w-[600px] shadow-2xl rounded-[32px] bg-white mb-20">
        <div class="text-center mb-8">
            <h3 id="pejabatTitle" class="text-2xl font-black text-slate-900">Tambah Pejabat</h3>
        </div>
        ...
        <form id="pejabatForm" action="<?= BASEURL; ?>/pejabat/tambah" method="POST" class="space-y-6 text-left">
            ...
            <div class="space-y-2">
                <label class="block text-xs font-bold text-gray-400 uppercase tracking-widest">ID Pejabat (samakan dengan id_user)</label>
            ...
            <div class="space-y-2">
                <label class="block text-xs font-bold text-gray-400 uppercase tracking-widest">Nama Pejabat</label>
...
<script>
    function openPejabatModal(mode, data = null) {
        ...
        if (mode === 'edit' && data) {
            ...
            title.innerText = 'Tambah Pejabat';
