...
<head>
    ...
    <style>
        ...
        table.report-table th, table.report-table td { padding: 8px 6px; text-align: left; border-bottom: 1px solid #f1f5f9; }
        table.report-table th { background-color: #f8fafc; font-weight: 800; text-transform: uppercase; text-align: center; color: #64748b; font-size: 7pt; }
...
<body>
    ...
    <table class="report-table">
        ...
        <tbody>
            <?php $no = 1; foreach($data['warga'] as $w): ?>
                <tr>
                    <td class="center"><?= $no++; ?></td>
                    <td class="center" style="font-family: monospace;"><?= $w['nik'] ?? ''; ?></td>
                    <td style="font-weight: 700;"><?= htmlspecialchars((string)($w['nama_lengkap'] ?? '')); ?></td>
                    <td class="center"><?= $w['jenis_kelamin'] ?? ''; ?></td>
                    <td><?= htmlspecialchars((string)($w['alamat'] ?? '')); ?></td>
                    <td><?= htmlspecialchars((string)($w['pekerjaan'] ?? '')); ?></td>
                </tr>
            <?php endforeach; ?>
        </tbody>
    </table>
