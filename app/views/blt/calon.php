    ...
    function openNilaiModal(idCalon, isDetail = false) {
        ...
        if (dataCalon && dataCalon.nilai) {
            ...
            kriteriaDivs.forEach(div => {
                const kriteriaId = parseInt(div.getAttribute('data-kriteria-id'));
                console.log('Processing kriteriaId:', kriteriaId, 'type:', typeof kriteriaId);
                ...
                if (input) {
                    if (dataCalon.nilai.hasOwnProperty(kriteriaId)) {
                        ...
                        console.log('Set input value (number key):', dataCalon.nilai[kriteriaId]);
                    } else {
                        // Check string key just in case
                        const kriteriaIdStr = String(kriteriaId);
                        if (dataCalon.nilai.hasOwnProperty(kriteriaIdStr)) {
                            input.value = dataCalon.nilai[kriteriaIdStr];
                            console.log('Set input value (string key):', dataCalon.nilai[kriteriaIdStr]);
                        }
                    }
                }
            });
